...
```
Para la componente gigante se estimaron la longitud promedio de los caminos mínimos (ASP) y el diámetro de la red. El valor obtenido para el ASP fue de aproximadamente 3.07, lo que indica una conectividad global elevada: en promedio, cualquier hotel puede alcanzarse desde otro mediante alrededor de tres saltos. Aunque este valor es mayor que en configuraciones más densas, sigue siendo bajo en relación con el tamaño de la red, lo que refleja una estructura razonablemente eficiente en términos de accesibilidad.

El diámetro de la red es 6, correspondiente al par de hoteles más alejados en términos topológicos. Este resultado sugiere que, incluso en los casos extremos, la separación entre establecimientos no es excesiva y no se observan patrones de fragmentación o aislamiento estructural.

En conjunto, estos indicadores describen una red hotelera bien integrada, donde la proximidad geográfica y el esquema de vecindad utilizado permiten mantener recorridos relativamente cortos entre nodos, garantizando una conectividad global adecuada dentro del sistema urbano.
...
```
Para identificar agrupamientos estructurales dentro de la red se aplicó el algoritmo de maximización de modularidad (greedy modularity). La modularidad cuantifica el contraste entre la densidad de conexiones internas de cada grupo y los enlaces que conectan nodos de distintas comunidades.

En la red analizada se identificaron tres comunidades principales, con un valor de modularidad Q = 0.53. Este valor indica la presencia de una estructura comunitaria moderada: las comunidades presentan una conectividad interna mayor que la esperada bajo un esquema aleatorio, aunque con fronteras menos marcadas que en configuraciones altamente modulares.

Las comunidades detectadas agrupan establecimientos de salud que, en términos generales, corresponden a zonas urbanas contiguas o funcionalmente relacionadas. Un primer grupo concentra hospitales y clínicas ubicados principalmente en sectores del norte y noroccidente de la ciudad, caracterizados por alta densidad de oferta y presencia de instituciones de alta complejidad. Un segundo grupo reúne establecimientos del suroccidente y municipios cercanos, mientras que el tercer conjunto agrupa hospitales del centro y sur de Bogotá, muchos de ellos asociados a servicios públicos y atención especializada.

En conjunto, estos resultados sugieren que la red hospitalaria presenta una segmentación espacial clara pero no extrema, donde la proximidad geográfica y la organización territorial influyen en la formación de comunidades, al tiempo que se preserva una conectividad global que integra el sistema de salud urbano.
