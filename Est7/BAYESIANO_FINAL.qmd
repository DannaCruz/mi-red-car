...
# Si tu sistema tiene problemas con tildes/espacios, renombra archivos a nombres simples
# y actualiza aquí. Ej: "siedco_delitos.xlsx"
```
...
# En tu archivo original, "reseñas" suele quedar como "rese_nas" con clean_names()
...
del_loc <- del_loc %>%
  ...
  mutate(
    ...
    # 👇 CLAVE: normaliza formato para que matchee con moteles/población
...
- Datos: \(s \sim \text{Binomial}(n,p)\)  
- Posterior: \(p \mid s \sim \text{Beta}(1+s,\;1+n-s)\)
